...
<div class="max-w-3xl mx-auto py-12 px-6 sm:px-10 font-['Inter']">
    <h2 class="text-4xl font-bold text-white mb-10 tracking-tight leading-tight">
        <span class="inline-block bg-gradient-to-r from-purple-400 to-pink-600 text-transparent bg-clip-text drop-shadow-md">✨ Tambah Produk Baru</span>
    </h2>

    <form method="POST" action="{{ route('products.store') }}" enctype="multipart/form-data" class="bg-[#0f172a] p-8 sm:p-10 rounded-3xl shadow-xl border border-gray-800 space-y-8 transition-all duration-300">
        @csrf
        ...
        <div>
            ...
            <textarea name="deskripsi" rows="4"
                class="w-full px-5 py-3 rounded-xl bg-gray-800 text-white border border-gray-700 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition">{{ old('deskripsi') }}</textarea>
        ...
        <div>
            <button type="submit"
                class="w-full py-3 rounded-xl bg-gradient-to-r from-indigo-600 to-pink-600 hover:from-indigo-700 hover:to-pink-700 text-white font-bold text-lg shadow-md transition-all">
                 Simpan Produk
            </button>
        </div>
    </form>
